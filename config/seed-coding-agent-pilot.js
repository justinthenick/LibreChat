...
async function resolveOwner(User) {
  ...
  if (admins.length > 1) {
    console.warn(`Multiple ADMIN users exist; assigning coding-agent pilot to the oldest admin (${admins[0]._id})`);
  }
  return admins[0];
}
...
async function seed() {
  ...
  if (!existing) {
    ...
    if (sameName.length > 1) {
      throw new Error(`Multiple admin-owned agents are named ${manifest.name}; refusing ambiguous adoption`);
    }
