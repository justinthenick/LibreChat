...
This Compose deployment explicitly enables scheduled chats and runs exactly one LibreChat API process, so `SCHEDULES_SINGLE_PROCESS=true` safely enables the process-local scheduler without Redis. Existing private `.env` files may omit the key because Compose supplies the safe single-process default; saving it through Admin Settings makes the assertion explicit. Before adding another API replica, set it to `false` and configure a shared Redis stream store; otherwise multiple schedulers cannot coordinate generation ownership safely.
